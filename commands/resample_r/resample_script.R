    ...
    for(i in plen){
      ...
      if(exten == "gt3x"){
        ...
        rtdat <- if(gtfz == vlist$resampfz){
          ...
          LUX <- PAADPh::round2(gsignal::resample(gtdat$LUX, gtfz, vlist$resampfz), 0); gc()
        ...
        mprocfunc <- function(nchunks){
          ...
          dat1 <- if(testdiv >= 2 & testdivint==T){
            ...
            rsampdat1 <- function(x){
              PAADPh::round2(gsignal::resample(x, vlist$resampfz, vlist$samprate), ndec)
            }
          ...
          dat1 <- if(vlist$matchadc=="False"){
            ...
            acl <- PAADPh::round2(seq((dynrange*-1), dynrange, ((2*dynrange)/(2^bitval))), 4)
